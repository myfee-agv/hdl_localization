...
#define GOOD_MATCHING_ERROR 0.1f
...
public:
  ...
  void onInit() override {
    ...
    localization_metric_pub = nh.advertise<std_msgs::Int32>("/localization_status", 5, false);
...
private:
  ...
  void initialize_params() {
    ...
    if (use_floor_filter) {
      ...
      pass->setFilterLimits(filter_height_min, filter_height_max);
      passthrough_filter = pass;
    }
...
private:
  ...
  void publish_scan_matching_status(const std_msgs::Header& header, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned) {
    ...
    // matching error (float): 0 = GOOD, 1 = BAD
    // inlier fraction (float): 0 = BAD, 1 = GOOD
    float matching = status.matching_error;
    if (matching > GOOD_MATCHING_ERROR){
        matching = GOOD_MATCHING_ERROR;
    }
    matching = matching / GOOD_MATCHING_ERROR;
    float average_error = (status.inlier_fraction + (1 - matching)) / 2.0;

    std_msgs::Int32 metric;
    metric.data = static_cast<int>(100 * average_error);    // Error in 0 - 100
    localization_metric_pub.publish(metric);
  }
...
private:
  ...
  ros::Publisher status_pub; // full debug info
  ros::Publisher localization_metric_pub;   // one number (0 - 100) defining the localization status
  ...
  pcl::Filter<PointT>::Ptr passthrough_filter;
